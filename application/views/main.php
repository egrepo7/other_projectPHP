<!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="UTF-8">
	<title>Login/Registration</title>
</head>
<body>
	<div id="container">
		<h2>Welcome!</h2>
		<form action="/Travels/register" method="POST">
			<fieldset>
				<legend>Register</legend>
				<?= form_error('reg_name'); ?>
				<p>Name:<input type="text" name="reg_name"></p>
				<?= form_error('reg_username'); ?>
				<p>Username:<input type="text" name="reg_username"></p>
				<?= form_error('reg_password'); ?>
				<p>Password:<input type="password" name="reg_password"></p>
				<?= form_error('reg_confirmpassword'); ?>
				<p>Confirm Password:<input type="password" name="reg_confirmpassword"></p>
				<input type="submit" value="Register">
			</fieldset>
		</form>

		<form action="/Travels/login" method="POST">
			<fieldset>
				<legend>Login</legend>
				<?= form_error('login_username'); ?>
				<p>Username:<input type="text" name="login_username"></p>
				<?= form_error('login_password'); ?>
				<p>Password:<input type="password" name="login_password"></p>
				<input type="submit" value="Login">
			</fieldset>
		</form>
	</div>
</body>
</html>